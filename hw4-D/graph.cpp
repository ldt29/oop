...
void Graph::InsertPlaceholder(const string &node_name) {
    ...
    PlaceholderNode* node=new PlaceholderNode(node_name);
...
void Graph::InsertConstant(const string &node_name, float value) {
    ...
    ConstantNode* node =new ConstantNode(node_name,value);
    nodes_.insert(node_name,node);
}
...
void Graph::InsertOperation(const string &node_name, const string &operation_type, 
    ...
    if(operation_type=="+"){
        Add* node = new Add(node_name,nodes_[input1],nodes_[input2]);
        nodes_.insert(node_name,node);
    }
    if(operation_type=="-"){
        Sub* node =new Sub(node_name,nodes_[input1],nodes_[input2]);
        nodes_.insert(node_name,node);
    }
}
...
void Graph::Eval(const string &node_name, const int &argument_num, 
    ...
    for(int i=0;i<argument_num;i++){
        PlaceholderNode* temp=dynamic_cast<PlaceholderNode* >(nodes_[argument_name_list_[i]]);
        temp->set_val(argument_value_list_[i]);
    }
